...
export default function SixthGeneration() {
  ...
  const sixthGeneration = React.useEffect(() => {
    ...
    //pokemon constants//
    const [correctName, setCorrectname] = React.useState('volcanion')
    const [number, setNumber] = React.useState(721)
    const [pokemonA, setPokemonA] = React.useState('bulbasaur')
    const [pokemonB, setPokemonB] = React.useState('volcanion')
    ...
    const play = async () => {
      ...
      try {
        ...
        const shuffled = await pokemons6.sort(() => 0.5 - Math.random())
